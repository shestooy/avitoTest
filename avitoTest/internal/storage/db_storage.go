...
func (p *DB) UpdateTender(ctx context.Context, tender model.Tender) (model.Tender, error) {
	...
	err = p.dbPool.QueryRow(ctx, `SELECT id, name, organization_id, description, status, servicetype, version, createdat FROM tenders
				WHERE id = $1 ORDER BY version DESC LIMIT 1`, tender.ID).Scan(&tender.ID, &tender.Name, &tender.OrganizationID,
...
func (p *DB) RollbackTender(ctx context.Context, tenderID, version, username string) (model.Tender, error) {
	...
	err = p.dbPool.QueryRow(ctx, `SELECT id, name, organization_id, description, status, servicetype, version, createdat FROM tenders
			WHERE version = $1`, version).Scan(&tender.ID, &tender.Name, &tender.OrganizationID,
		&tender.Description, &tender.Status, &tender.ServiceType, &tender.Version, &tender.CreatedAt)
	if err != nil {
		return model.Tender{}, err
	}
	err = p.dbPool.QueryRow(ctx, `SELECT version FROM tenders WHERE id = $1 ORDER BY version DESC`, tenderID).Scan(&tender.Version)
	if err != nil {
		return model.Tender{}, err
	}

	tender.Version++
	_, err = p.dbPool.Exec(ctx, `INSERT INTO tenders (id, name, organization_id, description, status, servicetype, version, createdat) VALUES 
				(uuid_generate_v4(), $1, $2, $3, $4, $5, $6, $7)`, tender.Name, tender.OrganizationID, tender.Description, tender.Description,
